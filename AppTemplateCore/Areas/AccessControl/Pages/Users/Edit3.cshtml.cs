...
namespace AppTemplateCore.Areas.AccessControl.Pages.Users
{
    public class EditModel3 : PageModel
    {
        ...
        {
            return Input.AllRolesList.Where(r => r.Selected == true).Select(s => s.Text).ToList().ToArray();
        }

        public List<Claim> GetSelectedClaims()
        {
            return Input.AllClaimsList.Where(c => c.Selected == true).Select(s => new Claim(s.Text, s.Value)).ToList();
        }


        public bool IsAnyRoleSelected()
        {
            return Input.AllRolesList.Any(r => r.Selected == true);
        }

        public bool IsAnyClaimSelected()
        {
            return Input.AllClaimsList.Any(c => c.Selected == true);
        }
        ...
        private async Task<bool> Load_Form_Reference_Data(ApplicationUser user)
        {
            ...
            Input.AllRolesList = RoleManager.Roles.ToList().Select(role => new SelectListItem()
            {
